...
function useConfigMapFormYaml(props) {
    ...
    React.useEffect(() => {
        axiosInstance
            .get("/admin/kubernetes/object/templates/values?kind=configmap")
            .then((response) => {
    ...
    const updateValues = (data, type) => {
        ...
        switch (type) {
            ...
            case 'data':
                ...
                break;
            case 'binaryData':
                yamlObject.binaryData = {}
                data.forEach(({ key, value }) => {
                    yamlObject.binaryData[key] = value;
                });
                break;
            default:
                break;
...
export default useConfigMapFormYaml;
