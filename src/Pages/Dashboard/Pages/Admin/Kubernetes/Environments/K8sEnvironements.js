...
export default function K8sEnvironements(props) {
  ...
  const columns = [
    ...
    },
    {
      ...
      flex: 2,
      renderCell: (params) => (<Link to={`/kubernetes/environment/${params.row.id}/edit`}>{params.row.name}</Link>)
    },
    {
      field: "description",
      headerName: counterpart("dashboard.k8sEnvironments.explore.table.description"),
      flex: 2,
    },
    {
      field: "created_at",
      headerName: counterpart("dashboard.k8sEnvironments.explore.table.creationDate"),
      flex: 2,
      renderCell: (params) => formateDate(params.row.created_at),
    },
    {
      field: "is_private",
      headerName: counterpart("dashboard.k8sEnvironments.explore.table.isPrivate"),
      flex: 2,
    },
    {
      field: 'action', headerName: counterpart("dashboard.k8sEnvironments.explore.table.actions"), flex: 2, renderCell: (params) => {
        const onClick = (e) => {
          e.stopPropagation();
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`/admin/environment/all?type=k8s`).then((res) => {
      setEnvs(res.data);
  ...
  const deleteEnvsHandler = async () => {
    ...
    new Promise((r, j) => {
      ...
      selectedDeletionItems.forEach((id, index) => {
        axios.delete(`/admin/kubernetes/environment/${id}`)
          .then(() => {
            deletedEnvs.push(id)
      ...
      .then((deleted_envs) => {
        ...
        if (deleted_envs.length > 0)
          toast.success(counterpart('dashboard.k8sEnvironments.explore.successDeleteAll'))
        setLoading(false)
  ...
  const deleteEnvHandler = () => {
    setLoading(true)
    axios.delete(`/admin/environment/kubernetes/${selectedEnv.id}`).then(response => {
      setEnvs(filteredListWithoutRemovedElement(selectedEnv.id, envs))
      setFilteredEnv(filteredListWithoutRemovedElement(selectedEnv.id, filteredEnv))
      toast.success(counterpart('dashboard.k8sEnvironments.explore.successDelete'))
      setShowConfirmDeleteModal(false)
      setLoading(false)
    }).catch(err => {
      setShowConfirmDeleteModal(false)
  ...
  return (
    ...
    >
        ...
        <Col>
          ...
          >
            <TextField
              ...
              placeholder={counterpart("dashboard.k8sEnvironments.explore.searchPlaceholder")}
              InputProps={{
                startAdornment: (
      ...
      <DataTable
        ...
        emptyMessage={counterpart("dashboard.k8sEnvironments.explore.emptyMessage")}
        createMessage={counterpart("dashboard.k8sEnvironments.explore.addEnvironement")}
        checkboxSelection
        columns={columns}
