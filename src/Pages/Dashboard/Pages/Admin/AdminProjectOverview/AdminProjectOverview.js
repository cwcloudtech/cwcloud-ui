...
import classes from "./AdminProjectOverview.module.css";
...
import TransferProjectModal from './TransferProjectModal/TransferProjectModal';
...
function AdminProjectOverview() {
    ...
    const onDeleteHandler = () => {
        ...
        axios.delete(`/admin/project/${project.id}`).then(response => {
    ...
    const instanceClickedHandler = (instance) => {
        ...
        navigate(`/admin/instance/${instance.id}`)
    }
    ...
    return (
        <div>
            ...
            <div className={classes.goBack} >
                <NavLink to='/admin/projects' className={classes.link} style={{color: colors.blue[_mode]}}>
                ...
                <Container className={classes.container} style={{backgroundColor: colors.secondBackground[_mode], border: '1px solid '+colors.border[_mode], borderRadius: "7px"}} fluid>
                    <Row style={{ marginLeft: "15px", marginRight: "15px" }}>
                        ...
                        <Col xs="12" md="4" className={classes.row}>
                            ...
                            <div style={{ display: "flex", alignItems: 'center' }}>
                                {!loading ?
                                    <Link style={{ cursor: 'pointer' }} to={`/user/${project.user.id}`}>{!loading ? project.user.email : <Skeleton width={100} />}</Link>
                                    :
